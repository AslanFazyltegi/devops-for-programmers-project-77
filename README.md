...
Для работы с проектом потребуется доступ к облачному провайдеру и настроенные ключи доступа для Terraform и Ansible. Данный проект выполнен в рамках обучения на hexlet.ru �и ссылка может не работать так как сервера выключны просто.
